<script lang="ts">
  ...
  const compileSCSS = async (scss: string, errorStore: typeof error1) => {
    if (!scss.trim()) {
      errorStore.set("-");
      return "";
    }
    try {
      ...
      errorStore.set(""); // Clear error if everything compiled

      return result.css;
    } catch (error: any) {
      // Extract the normal text of the Sass compilation error
      const errMsg = error.sassMessage || error.message || "Compilation error";
  ...
  $: compileSCSS(scss1, error1).then((result) => (css1 = result));
  $: compileSCSS(scss2, error2).then((result) => (css2 = result));
...
<style>
  ...
  .code__block--css:nth-child(1) {
    grid-area: css1;
  }
  .code__block--css:nth-child(2) {
    grid-area: css2;
  }
